...
.stat-card span{display:block;color:#9eb1ca;font-size:.9rem}.stat-card strong{display:block;font-size:2rem;margin:.35rem 0}.stat-card small{color:#7890ad}
...
<section class="stats-grid">
    ...
    <div class="card stat-card"><span>Tentatives</span><strong><?= $kpis['attempts'] ?></strong><small><?= $kpis['completed'] ?> quêtes terminées</small></div>
    ...
    <div class="card stat-card"><span>Badges débloqués</span><strong><?= $kpis['badges'] ?></strong><small>progression ludique</small></div>
</section>

<section class="card chart-card">
    <div class="chart-title">
        <div><h2>Activité sur les 14 derniers jours</h2><p>Nombre de nouvelles tentatives démarrées chaque jour.</p></div>
    </div>
    <div class="daily-chart" aria-label="Tentatives par jour">
        <?php foreach ($activity as $row):
            $height = max(2, (int)round(((int)$row['attempts'] / $maxDaily) * 100));
        ?>
...
<section class="card chart-card">
    <div class="chart-title">
        <div><h2>Activité par module</h2><p>Tentatives et progression moyenne, sans classement entre élèves.</p></div>
    </div>
    <div class="module-bars">
        <?php foreach ($moduleStats as $module):
            $attempts = (int)$module['attempts'];
            $width = $attempts > 0 ? max(3, (int)round(($attempts / $maxModuleAttempts) * 100)) : 0;
        ?>
            <div class="module-row">
                <div class="module-label">
                    <strong><?= e_admin_stats((string)($module['icon'] ?: '📘')) ?> <?= e_admin_stats((string)$module['title']) ?></strong>
                    <small><?= (int)$module['completed'] ?> terminée(s) · <?= (int)$module['game_over'] ?> game over</small>
                </div>
                <div>
                    <div class="bar-track"><div class="bar-fill" style="width:<?= $width ?>%"></div></div>
                    <div class="progress-stack"><div class="progress-line"><span>Progression moyenne</span><strong><?= (int)$module['avg_progress'] ?>%</strong></div></div>
                </div>
...
<section class="card chart-card">
    <div class="chart-title">
        <div><h2>Activité par classe</h2><p>Indicateurs collectifs uniquement : aucun classement individuel.</p></div>
    </div>
    <div class="stats-table-wrap">
        <table class="stats-table">
            <thead><tr><th>Classe</th><th>Élèves</th><th>Ont joué</th><th>Tentatives</th><th>Quêtes terminées</th><th>Badges</th></tr></thead>
            <tbody>
            <?php if (!$classStats): ?><tr><td colspan="6">Aucune activité élève pour le moment.</td></tr><?php endif; ?>
            <?php foreach ($classStats as $row): ?>
                <tr>
                    <td><strong><?= e_admin_stats((string)$row['class_code']) ?></strong></td>
                    <td><?= (int)$row['students'] ?></td>
                    <td><?= (int)$row['active_students'] ?></td>
                    <td><?= (int)$row['attempts'] ?></td>
                    <td><?= (int)$row['completed'] ?></td>
                    <td><?= (int)$row['badges'] ?></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
