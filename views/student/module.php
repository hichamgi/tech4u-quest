...
<section style="margin-top:1.25rem"><div style="display:flex;justify-content:space-between;align-items:end;gap:1rem;flex-wrap:wrap;margin-bottom:1rem"><div><h2 style="margin:0">Choisis ton mode</h2><p style="margin:.4rem 0 0;color:var(--muted)">Commence par le mode Facile. Chaque badge obtenu débloque le mode suivant.</p></div></div><div class="mode-grid">
...
<article class="card mode-card" style="<?= !$unlocked ? 'opacity:.55;filter:saturate(.65);' : '' ?>"><div class="mode-title"><span class="mode-icon"><?= e((string)($path['icon'] ?: '🎯')) ?></span><h3>Mode <?= e($mode) ?></h3></div><p class="mode-description"><?= e((string)($path['description'] ?? '')) ?></p><div class="stats-row" style="margin:.9rem 0"><div class="stat-mini"><strong><?= (int)$path['question_count'] ?></strong><span>questions</span></div></div>
<?php if ($badgeObtained): ?><div class="chip" style="margin-bottom:.75rem">🏅 Badge <?= e($mode) ?> obtenu</div><?php elseif (!$unlocked): ?><div class="chip" style="margin-bottom:.75rem">🔒 Badge du mode précédent requis</div><?php elseif ($currentAttempt > 0): ?><div class="chip" style="margin-bottom:.75rem">▶️ Partie en cours</div><?php elseif ($completed): ?><div class="chip" style="margin-bottom:.75rem">✅ Mode terminé</div><?php endif; ?>
<div class="mode-footer"><?php if ($unlocked): ?><form method="post" action="<?= e(Url::to('module/' . (int)$module['id'] . '/start')) ?>"><input type="hidden" name="csrf_token" value="<?= e((string)$csrfToken) ?>"><input type="hidden" name="path_id" value="<?= (int)$path['id'] ?>"><button class="btn <?= $code==='expert' ? 'btn-gold' : 'btn-primary' ?>" type="submit" style="width:100%"><?= $currentAttempt > 0 ? 'Continuer le mode ' . e($mode) : ($badgeObtained ? 'Rejouer le mode ' . e($mode) : 'Lancer le mode ' . e($mode)) ?></button></form><?php else: ?><button class="btn btn-secondary" type="button" disabled style="width:100%;cursor:not-allowed">🔒 Mode <?= e($mode) ?> verrouillé</button><?php endif; ?></div></article>
<?php endforeach; ?></div></section>
<section class="card card-pad" style="margin-top:1.25rem"><h2>Progression et badges</h2><p style="color:var(--muted);line-height:1.7">Les badges sont désactivés au départ. Chaque fois que tu termines un mode, son badge devient actif et débloque le mode suivant.</p><div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:1rem;margin-top:1rem"><?php foreach ($paths as $path): $code=(string)$path['code'];$mode=$modeLabels[$code]??(string)$path['name'];$badgeObtained=(int)($path['badge_obtained']??0)===1; ?><div class="card" style="padding:1rem;text-align:center;<?= $badgeObtained ? '' : 'opacity:.42;filter:grayscale(1);' ?>"><div style="font-size:2.2rem;margin-bottom:.4rem"><?= e((string)($path['icon'] ?: '🏅')) ?></div><strong>Badge <?= e($mode) ?></strong><div style="margin-top:.55rem"><span class="chip"><?= $badgeObtained ? '✅ Obtenu' : '🔒 Non obtenu' ?></span></div></div><?php endforeach; ?></div></section>
<?php endif; ?>
</main>
