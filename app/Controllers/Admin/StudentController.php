...
final class StudentController
{
    public function index(): void
    {
        ...
        if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
            if (!Auth::validateCsrf($_POST['csrf_token'] ?? null)) {
                ...
                try {
                    ...
                    if (!$s->fetchColumn()) throw new RuntimeException('Le compte DEMO-1 (ID 2) n’existe pas. Importe-le d’abord avec le CSV élèves.');
        ...
        $demo = $db->prepare('SELECT s.id,s.login_code,s.active,(SELECT COUNT(*) FROM attempts a WHERE a.student_id=s.id) attempts,(SELECT COUNT(*) FROM student_badges sb WHERE sb.student_id=s.id) badges FROM students s WHERE s.id=:id AND s.class_code=:class AND s.student_number=:number LIMIT 1');
