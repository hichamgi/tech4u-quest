...
final class GameService
{
    ...
    {
        $quota=$this->db->prepare('SELECT c.id AS category_id,mcs.question_count FROM module_category_settings mcs JOIN categories c ON c.id=mcs.category_id WHERE mcs.module_id=:module AND c.active=1 AND mcs.question_count>0 ORDER BY c.display_order,c.id');$quota->execute(['module'=>$moduleId]);$rows=$quota->fetchAll(PDO::FETCH_ASSOC);$questionIds=[];$usedGroups=[];
        foreach($rows as $row){$cid=(int)$row['category_id'];$needed=(int)$row['question_count'];for($i=0;$i<$needed;$i++){
            $sql='SELECT id,exclusion_group FROM questions WHERE category_id=:category AND active=1';$params=['category'=>$cid];
            if($usedGroups){$marks=[];foreach(array_values($usedGroups) as $j=>$g){$k='g'.$j;$marks[]=':'.$k;$params[$k]=$g;}$sql.=' AND (exclusion_group IS NULL OR trim(exclusion_group)="" OR exclusion_group NOT IN ('.implode(',',$marks).'))';}
            if($questionIds){$marks=[];foreach($questionIds as $j=>$qid){$k='q'.$j;$marks[]=':'.$k;$params[$k]=$qid;}$sql.=' AND id NOT IN ('.implode(',',$marks).')';}
            $sql.=' ORDER BY RANDOM() LIMIT 1';$s=$this->db->prepare($sql);$s->execute($params);$q=$s->fetch(PDO::FETCH_ASSOC);if(!$q)throw new RuntimeException("Catégorie {$cid} : impossible de respecter le quota ({$needed}) avec les groupes d’exclusion configurés.");$questionIds[]=(int)$q['id'];$g=trim((string)($q['exclusion_group']??''));if($g!=='')$usedGroups[$g]=$g;
        }}
    ...
    {
        $stmt=$this->db->prepare('SELECT a.*,m.title AS module_title,m.icon AS module_icon,b.name AS badge_name,b.icon AS badge_icon FROM attempts a JOIN modules m ON m.id=a.module_id LEFT JOIN badges b ON b.module_id=m.id WHERE a.id=:id AND a.student_id=:student');$stmt->execute(['id'=>$attemptId,'student'=>$studentId]);$attempt=$stmt->fetch(PDO::FETCH_ASSOC);if(!$attempt)throw new RuntimeException('Tentative introuvable.');return $attempt;
    }
    ...
    {
        $question=$this->currentQuestion($attemptId,$studentId);$attempt=$question['attempt'];$type=(string)$question['type'];$answerStmt=$this->db->prepare('SELECT id,answer,is_correct FROM question_answers WHERE question_id=:question ORDER BY display_order,id');$answerStmt->execute(['question'=>$question['id']]);$answers=$answerStmt->fetchAll(PDO::FETCH_ASSOC);$isCorrect=false;$answerId=null;$shortAnswer=null;
        ...
        $this->db->beginTransaction();try{$log=$this->db->prepare('INSERT INTO attempt_answers(attempt_id,attempt_question_id,answer_id,short_answer,is_correct) VALUES(:attempt,:aq,:answer_id,:short_answer,:correct)');$log->execute(['attempt'=>$attemptId,'aq'=>$question['attempt_question_id'],'answer_id'=>$answerId?:null,'short_answer'=>$shortAnswer,'correct'=>$isCorrect?1:0]);if($isCorrect){$this->db->prepare('UPDATE attempt_questions SET answered=1,completed=1 WHERE id=:id')->execute(['id'=>$question['attempt_question_id']]);$newScore=(int)$attempt['score']+1;$position=(int)$attempt['current_position'];$total=(int)$attempt['total_questions'];if($position>=$total){$this->db->prepare('UPDATE attempts SET score=:score,status="completed",finished_at=CURRENT_TIMESTAMP WHERE id=:id')->execute(['score'=>$newScore,'id'=>$attemptId]);$this->awardBadge($studentId,(int)$attempt['module_id'],$attemptId);$this->db->commit();return['correct'=>true,'status'=>'completed','attempt_id'=>$attemptId];}$this->db->prepare('UPDATE attempts SET score=:score,current_position=current_position+1 WHERE id=:id')->execute(['score'=>$newScore,'id'=>$attemptId]);$this->db->commit();return['correct'=>true,'status'=>'in_progress','attempt_id'=>$attemptId];}
        $newLives=max(0,(int)$attempt['lives']-1);$this->db->prepare('UPDATE attempt_questions SET answered=1,wrong_answers=wrong_answers+1 WHERE id=:id')->execute(['id'=>$question['attempt_question_id']]);if($newLives===0){$this->db->prepare('UPDATE attempts SET lives=0,status="game_over",finished_at=CURRENT_TIMESTAMP WHERE id=:id')->execute(['id'=>$attemptId]);$this->db->commit();return['correct'=>false,'status'=>'game_over','attempt_id'=>$attemptId];}$this->db->prepare('UPDATE attempts SET lives=:lives WHERE id=:id')->execute(['lives'=>$newLives,'id'=>$attemptId]);$this->db->commit();return['correct'=>false,'status'=>'in_progress','attempt_id'=>$attemptId,'lives'=>$newLives];}catch(\Throwable $e){if($this->db->inTransaction())$this->db->rollBack();throw $e;}
    }
