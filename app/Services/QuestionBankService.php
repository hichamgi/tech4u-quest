...
final class QuestionBankService
{
    ...
    public function save(array $data, ?int $questionId = null): int
    {
        ...
        foreach ($answers as $a) {
            ...
            $text = trim((string)($a['answer'] ?? ''));
            if ($text === '') continue;
            $clean[] = ['answer'=>$text, 'is_correct'=>!empty($a['is_correct']) ? 1 : 0];
        }
        ...
        if ($dup->fetchColumn()) throw new RuntimeException('Une question identique existe déjà.');

        $this->db->beginTransaction();
        try {
            if ($questionId) {
                $stmt = $this->db->prepare('UPDATE questions SET category_id=:c,question=:q,type=:t,difficulty=:d,lesson=:l,topic=:p,explanation=:e,active=:a,updated_at=CURRENT_TIMESTAMP WHERE id=:id');
                $stmt->execute(['c'=>$categoryId,'q'=>$question,'t'=>$type,'d'=>$difficulty,'l'=>$lesson ?: null,'p'=>$topic ?: null,'e'=>$explanation ?: null,'a'=>$active,'id'=>$questionId]);
                $this->db->prepare('DELETE FROM question_answers WHERE question_id=:id')->execute(['id'=>$questionId]);
                $id = $questionId;
            } else {
                $stmt = $this->db->prepare('INSERT INTO questions(category_id,question,type,difficulty,lesson,topic,explanation,active) VALUES(:c,:q,:t,:d,:l,:p,:e,:a)');
                $stmt->execute(['c'=>$categoryId,'q'=>$question,'t'=>$type,'d'=>$difficulty,'l'=>$lesson ?: null,'p'=>$topic ?: null,'e'=>$explanation ?: null,'a'=>$active]);
    ...
    {
        $q = $this->db->prepare('SELECT * FROM questions WHERE id=:id'); $q->execute(['id'=>$id]); $row=$q->fetch(PDO::FETCH_ASSOC);
        if (!$row) throw new RuntimeException('Question introuvable.');
        $a=$this->db->prepare('SELECT answer,is_correct FROM question_answers WHERE question_id=:id ORDER BY display_order,id'); $a->execute(['id'=>$id]);
        $answers=$a->fetchAll(PDO::FETCH_ASSOC);
        return $this->save(['category_id'=>$row['category_id'],'question'=>$row['question'].' (copie)','type'=>$row['type'],'difficulty'=>$row['difficulty'],'lesson'=>$row['lesson'],'topic'=>$row['topic'],'explanation'=>$row['explanation'],'active'=>0,'answers'=>$answers]);
    }
