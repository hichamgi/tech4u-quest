...
final class StatisticsService
{
    ...
    public function build(): array
    {
        ...
        foreach ($classLevelRows as $row) {
            ...
            if (!isset($classLevelStats[$class])) {
                $classLevelStats[$class] = [];
            }
            $classLevelStats[$class][(string)$row['code']] = $row;
        }

        $studentLevelRows = $this->db->query(
        ...
        $kpis = [
            'students' => (int)($kpiRow['students'] ?? 0),
            'active_students' => (int)($kpiRow['active_students'] ?? 0),
            'attempts' => (int)($kpiRow['attempts'] ?? 0),
            'completed' => (int)($kpiRow['completed'] ?? 0),
            'game_over' => (int)($kpiRow['game_over'] ?? 0),
            'badges' => $badgeCount,
            'answers' => $answers,
